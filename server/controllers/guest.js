...
module.exports = {
    ...
    filteredList(req, res) {
        Guest.findAll({
            ...
            attributes: ['id', 'name', 'response', 'plusonelist', 'invitationsent', [sequelize.fn("concat", "www.fernandezcanowedding.com/guest/", sequelize.fn('MD5', sequelize.cast(sequelize.col("id"), 'text'))), 'url']]
        }).then(guests => res.status(200).send(guests)).catch(error => res.status(400).send(error));
    },
    sendemail(req, res) {
        Guest.findAll({
